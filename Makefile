...
# core の検査は consumer 相当の project 層を前提にする — fresh clone では先に `make scaffold`。
...
# kit venv (uv sync --project core が $(CURDIR)/.venv に作る。core/Makefile の $(PY) と同一実体)。
PY := $(CURDIR)/.venv/bin/python

CHECKS := check-core check-install-sh check-templates
.PHONY: help check scaffold $(CHECKS)
...
PROJECT_NAME ?= kitci
scaffold: ## consumer 相当の project 層を templates から用意 (既存ファイルは触らない)
